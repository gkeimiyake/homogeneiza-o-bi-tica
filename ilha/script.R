...
teste.shapiro
check_homogeneity(data="comunidades", dependent="especies", group="tipo_restinga", method="levene")
leveneTest(especies~tipo_restinga,data = comunidades)
leveneTest(abundancia~tipo_restinga, data=comunidades)
teste.t<-t.test(comunidades$especies~comunidades$tipo_restinga)
teste.t
...
teste.shapiro_a
teste.t_a<-t.test(comunidades$abundancia~comunidades$tipo_restinga)
teste.t_a
...
plot(density(beta.restinga_aberta$sampled.values$beta.BRAY), col="red", xlim=c(0,1))
lines(density(beta.restinga_fechada$sampled.values$beta.BRAY), col="blue")
...
plot(density(beta.restinga_aberta$sampled.values$beta.BRAY), 
     ...
     ylab = "Densidade")
?density
# Add the density line for "Restinga Fechada"
lines(density(beta.restinga_fechada$sampled.values$beta.BRAY), col = "grey30",
      lwd=2)

# Add a subtitle indicating the labels for the lines
legend("right",                # Position of the legend
       legend = c("Restinga Aberta", "Restinga Fechada"), # Labels
       col = c("grey70", "grey30"),    # Line colors
